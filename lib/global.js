...
export function GlobalContextProvider(props) {
  ...
  const toggleDarkMode = () => {
    ...
    footers.classList?.remove(newStatus ? 'bg-white' : 'bg-black')
    footers.classList?.add(newStatus ? 'bg-black' : 'bg-white')
  
    // 修改页脚文字颜色
    const textElements = footers.querySelectorAll('.footer-text')
    textElements.forEach(el => {
      el.classList.remove(newStatus ? 'text-black' : 'text-white')
      el.classList.add(newStatus ? 'text-white' : 'text-black')
    })
    // 修改AI摘要颜色
    if (newStatus) {
      // 如果 newStatus 为真，执行相应的代码
    } else {
      const summaryBox = document.getElementById('post-ai');
      summaryBox.style.backgroundColor = '#474848'; 
      
      const aiTitleDiv = document.getElementById('ai-title');
      aiTitleDiv.style.color = '#ffffff'; // 将字体颜色设置为白色

      const aiSpeechContent = summaryBox.querySelectorAll('.ai-speech-content');
      aiSpeechContent.forEach(element => {
        element.style.backgroundColor = '#374151'; // 设置背景颜色
        element.style.color = '#ffffff'; // 将字体颜色设置为白色
      });
    }
